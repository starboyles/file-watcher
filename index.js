...
program
  ...
  .option('-i, --ignore <ignore>', 'Ignore pattern (comma-separated)', 'node_modules/**/*')
  .parse(process.argv);
...
watcher
  .on('add', path => console.log(chalk.green(`📝 Added: ${path.replace(process.cwd(), '')}`)))
  .on('change', path => console.log(chalk.yellow(`🔄 Changed: ${path.replace(process.cwd(), '')}`)))
  .on('unlink', path => console.log(chalk.red(`🗑️  Removed: ${path.replace(process.cwd(), '')}`)))
